...
const createCompetition = asyncWrapper(async (req, res) => {
    ...
    // Determine thumbnail — file upload takes priority, then JSON URL string fallback
    let thumbnailData;
    if (req.file) {
        try {
            thumbnailData = await uploadToCloud(req.file.path);
        } catch (err) {
            cleanupUploadedFiles(req);
            throw err;
        }
    } else if (image && typeof image === 'string') {
        thumbnailData = { publicId: image, secureUrl: image };
    } else if (thumbnailUrl && typeof thumbnailUrl === 'string') {
        thumbnailData = { publicId: thumbnailUrl, secureUrl: thumbnailUrl };
    } else {
...
const updateCompetition = asyncWrapper(async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(competitionId)) {
        cleanupUploadedFiles(req);
        throw new BAD_REQUEST('Invalid Competition ID format');
    }
    ...
    if (req.file) {
        ...
        try { await deleteFromCloud(comp.thumbnail?.publicId); } catch {}
        newThumbnail = cloudResult;
    } else if (image && typeof image === 'string') {
        newThumbnail = { publicId: image, secureUrl: image };
    } else if (thumbnailUrl && typeof thumbnailUrl === 'string') {
        newThumbnail = { publicId: thumbnailUrl, secureUrl: thumbnailUrl };
    }
