...
const addSection = asyncWrapper(async (req, res) => {
    const { heading, description, blocks, image, thumbnail, sectionImage } = req.body;

    let cloudResult;
    const imageUrl = image || thumbnail || sectionImage || req.body?.secureUrl;
    ...
    if (req.file) {
        ...
        cloudResult = {
            secureUrl: imageUrl,
            publicId: 'sample_product_section_' + Date.now()
        };
    } else {
        cleanupUploadedFiles(req);
...
const updateSection = asyncWrapper(async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(sectionId)) {
        cleanupUploadedFiles(req);
        throw new BAD_REQUEST('Invalid Section ID format');
    }
